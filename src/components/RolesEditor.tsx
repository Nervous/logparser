...
export default function RolesEditor({ superAdmin }: { superAdmin: boolean }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (res.ok) { setLogTypes(j.logTypes); setRoles(j.roles); }
      setLoading(false);
    })();
  }, []);
  ...
  async function toggle(role: Role, key: string, next: boolean) {
    ...
    await fetch("/api/roles", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ roleId: role.id, logTypeKey: key, allowed: next }) });
    setSaving(null);
  }
  async function toggleSeeAll(role: Role, next: boolean) {
    ...
    await fetch("/api/roles", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ roleId: role.id, seeAll: next }) });
    setSaving(null);
  }
